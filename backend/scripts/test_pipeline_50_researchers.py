#!/usr/bin/env python3
"""
Test pipeline with 50 Texas State University researchers.
This script implements the core data pipeline to fetch, process, and store
researcher data from OpenAlex API into the Supabase database.
"""
import os
import sys
import time
...
class ResearcherPipeline:
    """Main pipeline for processing researcher data."""
    ...
    def process_researchers(self, institution_openalex_id: str, limit: int = 50) -> List[str]:
        """
        ...
        try:
            ...
            for work_data in self.openalex_client.get_works_by_author(researcher_openalex_id):
                try:
                    ...
                    # Process topics - this is the key fix
                    topics_data = work_data.get('topics', [])
                    if topics_data:
                        logger.debug(f"Found {len(topics_data)} topics for work: {work_data.get('title', 'Unknown')[:50]}...")
                        ...
                        if processed_topics:
                            ...
                            logger.debug(f"Stored {len(processed_topics)} topics for work {work_id}")
                        else:
                            logger.debug(f"No valid topics processed for work {work_id}")
                    else:
                        logger.debug(f"No topics found for work: {work_data.get('title', 'Unknown')[:50]}...")
    ...
    def run_pipeline(self, limit: int = 50):
        ...
        """
        try:
            logger.info(f"Starting pipeline test with {limit} researchers")
    ...
    def generate_report(self):
        ...
        try:
            ...
            print(f"  Topics Processed: {metrics_summary['records_processed']['topics']}")
            ...
            print(f"\n⚠️  ERROR METRICS:")
            print(f"  Total Errors: {metrics_summary['records_processed']['researchers'] + metrics_summary['records_processed']['works']}")
            ...
            print(f"\n✅ SUCCESS CRITERIA EVALUATION:")
            success_criteria = {
                "50 researchers processed": metrics_summary['records_processed']['researchers'] >= 50,
